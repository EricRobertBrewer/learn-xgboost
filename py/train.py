...
def print_lines(path, line_indices):
  with open(path, 'r', encoding='utf-8') as fd:
    n_lines = 0
    i = 0
    while i < len(line_indices):
      line = fd.readline()[:-1]
      if line_indices[i] == n_lines:
        print(line)
        i += 1
      n_lines += 1
...
def main():
  ...
  with mlflow.start_run():
    ...
    # false positives
    fp = (predict_labels == 1) & (labels == 0)
    print("{:d} false positives".format(sum(fp)))
    fpi = [i for i, value in enumerate(fp) if value]
    print_lines('data/pairs2.psv', fpi)
    # false negatives
    fn = (predict_labels == 0) & (labels == 1)
    print("{:d} false negatives".format(sum(fn)))
    fni = [i for i, value in enumerate(fn) if value]
    print_lines('data/pairs2.psv', fni)
    ...
    plt.savefig("artifacts/tree.png", dpi=400)
    plt.show()
